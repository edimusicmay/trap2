...
// Spawn spaceship every 1s during long notes
if (isAnyHoldActive() && performance.now() - lastSpaceshipSpawnTime > 1000) {
  ...
  lastSpaceshipSpawnTime = performance.now();
}
...
spaceships.forEach(ship => {
  if (ship.clicked && performance.now() - ship.clickTime > 1000) return;
  ...
  if (ship.clicked) {
    ...
    ctx.drawImage(spaceshipImage, -20, -20, 80, 80);
  }
...
activeTouchLanes.clear(); // clear any previous state
...
for (let touch of e.touches) {
  ...
  let touchedSpaceship = false;
  for (let ship of spaceships) {
    const dx = ship.x - touchX;
    const dy = ship.y - touchY;
    const dist = Math.sqrt(dx * dx + dy * dy);
if (dist < 40 && !ship.clicked) {

      ship.clicked = true;
      ship.clickTime = performance.now();
      score += 100;
      touchedSpaceship = true;
      break;
    }
  }

  if (touchedSpaceship) continue;

  const laneWidth = canvas.width / 4;
  const screenX = touch.clientX - rect.left;
  const lane = Math.floor(screenX / laneWidth);
  if (lane >= 0 && lane < 4) {
    activeTouchLanes.add(lane); // Track it
    const key = 'asdf'[lane];
...
canvas.addEventListener('touchend', (e) => {
  e.preventDefault();
for (let lane of activeTouchLanes) {
  const key = 'asdf'[lane];
  keyHeld[key] = false;
