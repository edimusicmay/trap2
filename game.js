...
let activeTouches = {};  // { identifier: laneIndex }
...
if (isAnyHoldActive() && performance.now() - lastSpaceshipSpawn > 1000) {
  lastSpaceshipSpawn = performance.now();
  spaceships.push({
...
spaceships.forEach(ship => {
  ...
  const size = 180; // 60 × 3
  ctx.drawImage(spaceshipImage, ship.x - size / 2, ship.y - size / 2, size, size);
});

// Show +100 popups
spaceshipPopups.forEach(p => {
  const age = performance.now() - p.time;
  if (age < 1000) {
    ctx.font = 'bold 24px Arial';
    ctx.fillStyle = yellow;
    ctx.textAlign = 'center';
    ctx.fillText('+100', p.x, p.y - age * 0.1);
  }
});

// Cleanup
spaceshipPopups = spaceshipPopups.filter(p => performance.now() - p.time < 1000);
spaceships = spaceships.filter(ship =>
  ship.x >= -100 && ship.x <= BASE_WIDTH + 100 &&
  ship.y >= -100 && ship.y <= BASE_HEIGHT + 100
);
...
canvas.addEventListener('touchend', (e) => {
  ...
  for (let changedTouch of e.changedTouches) {
    const touchId = changedTouch.identifier;
    const lane = activeTouches[touchId];
    ...
    if (lane !== undefined) {
      ...
      delete activeTouches[touchId];  // Only remove this finger's lane
    }
  }
}, { passive: false });
...
canvas.addEventListener('touchstart', (e) => {
  ...
  for (let touch of e.touches) {
    const touchX = (touch.clientX - rect.left) * BASE_WIDTH / canvas.width;
    const touchY = (touch.clientY - rect.top) * BASE_HEIGHT / canvas.height;

    let tappedSpaceship = false;
    for (let ship of spaceships) {
      if (ship.alive) {
        const dx = ship.x - touchX;
        const dy = ship.y - touchY;
        const dist = Math.sqrt(dx * dx + dy * dy);
        if (dist < 90) { // Adjusted hit radius to match drawing size
          ship.alive = false;
          score += 100;
          spaceshipPopups.push({ x: ship.x, y: ship.y, time: performance.now() });
          tappedSpaceship = true;
          break;
        }
      }
    }
...
if (!tappedSpaceship) {
  ...
  if (lane >= 0 && lane < 4) {
    ...
    // Prevent re-trigger if already touching that lane
    const alreadyUsed = Object.values(activeTouches).includes(lane);
    if (!alreadyUsed) {
...
canvas.addEventListener('click', (e) => {
  const rect = canvas.getBoundingClientRect();
  const clickX = (e.clientX - rect.left) * BASE_WIDTH / canvas.width;
  const clickY = (e.clientY - rect.top) * BASE_HEIGHT / canvas.height;

  for (let ship of spaceships) {
    if (ship.alive) {
      const dx = ship.x - clickX;
      const dy = ship.y - clickY;
      const dist = Math.sqrt(dx * dx + dy * dy);
      if (dist < 90) {
        ship.alive = false;
        score += 100;
